...
>
> [消息认证码与哈希算法的区别 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/398292957#:~:text=Hash与MAC的区别，Hash只能保证消息的完整性，MAC不仅能够保证完整性，还能够保证真实性。,比如A想给B发送一条消息，A需要把消息内容和对应的消息摘要都发给B；B通过同样的摘要算法计算摘要，就可以知道消息是否被篡改。)
>
...
> [HMAC算法原理 - 月染霜华 - 博客园 (cnblogs.com)](https://www.cnblogs.com/shoshana-kong/p/11497676.html#:~:text=HMAC算法的定义. HMAC算法是一种执行“校验和”的算法，它通过对数据进行“校验”来检查数据是否被更改了。. 在发送数据以前，HMAC算法对数据块和双方约定的公钥进行“散列操作”，以生成称为“摘要”的东西，附加在待发送的数据块中。.,当数据和摘要到达其目的地时，就使用HMAC算法来生成另一个校验和，如果两个数字相匹配，那么数据未被做任何篡改。. 否则，就意味着数据在传输或存储过程中被某些居心叵测的人作了手脚。. HMAC算法的定义用公式表示如下：. HMAC（K，M）%3DH（（K’⊕opad）∣H（（K’⊕ipad）∣M））.)
>
> [HMAC算法及其应用 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/136590049)
...
1. 将步骤 1 的结果跟 ipad 做异或。
1. 将要加密的信息附在步骤 2 的结果后面
1. 调用 H 方法计算步骤 3 的结果
1. 将步骤 1 的结果跟 opad 做异或。 
1. 将步骤 4 的结果附在步骤 5 的结果后面。 
1. 调用 H 方法计算步骤 6 的结果
